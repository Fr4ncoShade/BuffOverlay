...
--==============
local function RunNextFrame(func)
...
local spellTooltip = CreateFrame("GameTooltip", "BuffOverlaySpellTooltip", nil, "GameTooltipTemplate")
...
function BuffOverlay:GetIconString(icon, iconSize)
    ...
    return format(
        "|T%s:%d:%d:0:0:256:256:20:236:20:236|t",
        icon,
        size,
        size
    )
            ...
            if not v.parent and (v.class == class) then
                ...
                spells[spellIdStr] = {
                    ...
                    args = {
                        ...
                        edit = {
                            name = "",
							image = "Interface\\AddOns\\BuffOverlay\\Media\\Textures\\UI-OptionsButton",
                            imageWidth = 12,
                            imageHeight = 12,
                            type = "execute",
                            order = 1,
            ...
            settings = {
                ...
                args = {
                    ...
                    showTooltip = {
						...
						desc = L["Toggle showing of the tooltip when hovering over an icon."]
							.. "\n\n|cffff2020"
							--.. L["Warning: when enabled, overlay icons will block mouse clicks on the unit frame beneath them."]
							.. "Warning: when enabled, overlay icons will block mouse clicks on the unit frame beneath them."
							.. "|r",
                    },
                },
            },
...
local customSpellInfo = {
    ...
    delete = {
        ...
        func = function(info)
			...
			local text = format(
				"%s\n\n%s %s\n\n",
				L["Are you sure you want to delete this spell?"],
				BuffOverlay:GetIconString(icon, 20),
				spellName or tostring(spellId)
			)
            if BuffOverlay.defaultSpells[spellId] then
	...
	icon = {
		...
		type = "input",
		width = 1.1,
		desc = L["The texture path to use for this spell. Example: Interface\\Icons\\Spell_Holy_Renew"],
		...
		set = function(info, state)
			...
			local name, _, defaultIcon = GetSpellInfo(spellId)

			-- Trim whitespace
			state = state:gsub("^%s+", ""):gsub("%s+$", "")

			-- Empty input resets custom icon
			if state == "" then
				BuffOverlay.db.global.customBuffs[spellId][option] = nil

				BuffOverlay.options.args.customSpells.args[spellIdStr].name =
					format("%s %s", BuffOverlay:GetIconString(defaultIcon, 15), name)

			else
				-- Optional sanity check
				if not state:find("Interface\\") then
					BuffOverlay:Print(format(
						L["Invalid texture path: %s"],
						BuffOverlay:Colorize(state)
					))
					return
				end
				...
				BuffOverlay.options.args.customSpells.args[spellIdStr].name =
					format("%s %s", BuffOverlay:GetIconString(state, 15), name)
			end
