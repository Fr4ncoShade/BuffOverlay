...
function BuffOverlay:AddBar()
    ...
    if Masque then --================================Button Facade или Masque
...
local function UnitAuraTest(unit, index, filter)
    if testSingleAura then
        ...
        if index == 1 and next(testBuffs) == nil then
            for spellId, aura in pairs(BuffOverlay.db.profile.buffs) do
                if aura and not aura.parent then
...
function BuffOverlay:InsertCustomAura(spellId)
	if not SpellExists(spellId) then
		return false
	end
...
function BuffOverlay:InsertCustomChild(childId, parentId)
	if not SpellExists(childId) then
...
function BuffOverlay:OnInitialize()
    ...
    self.eventHandler:SetScript("OnEvent", function(_, event)
        if event == "PLAYER_LOGIN" then
			self.pixelFactor = GetPixelFactor()
            self:InitFrames()
...
function BuffOverlay:RefreshOverlays(full, barName)
    ...
    for frame in pairs(self.blizzFrames) do
        if frame:IsShown() then
			local unit = (self.frames[frame] and self.frames[frame].unit) or frame.unit
            self:ApplyOverlay(frame, unit, barName)
        else
            HideAllOverlays(frame)
        end
...
function BuffOverlay:Test(barName, singleAura)
    ...
    if not self.test then
        if not testTextFrame then
            ...
            testTextFrame.bg:SetTexture(1, 0, 0, 0.6)
...
--
		...
		-- Fallback
		if not anchor and PlayerFrame
			and PlayerFrame:IsShown()
			and PlayerFrame:IsVisible()
		then
			anchor = PlayerFrame
		end
...
local function SetOverlayAura(overlay, index, icon, count, duration, expirationTime, dispelType, filter, spellId)
	...
	if enabled and bar.showCooldownSpiral then
		local startTime = expirationTime - duration

		if CooldownFrame_Set then
			CooldownFrame_Set(overlay.cooldown, startTime, duration, true)
		elseif overlay.cooldown.SetCooldown then
...
local function UpdateBorder(overlay)
    ...
    -- Zoom icon texture when border is enabled
    ...
    if not overlay.border then
        overlay.border = CreateFrame(
            "Frame",
            nil,
            overlay,
            "BuffOverlayBorderTemplate"
        )
        ...
        if border.SetFrameLevel then
            border.SetFrameLevel = nop
        end

        -- Call OnLoad manually for WotLK compatibility
        if border.OnLoad then
            border:OnLoad()
        end
    end
...
function BuffOverlay:ApplyOverlay(frame, unit, barNameToApply)
    ...
    for barName, bar in pairs(bars) do
        if ShouldShow(bar, frameType) and not (barNameToApply and barName ~= barNameToApply) then
            if Masque and not bar.group then -------------------------==========================================Masque или ButtonFacade
                bar.group = Masque:Group("BuffOverlay", bar.name, barName)
                bar.group:RegisterCallback(masqueCallback) --=========================?  RegisterSkinCallback()
            ...
            for i = 1, bar.iconCount do
                ...
                then
					...
					if not overlay then
						overlay = CreateFrame("Button", overlayName .. i, frame.BuffOverlays)
						...
						overlay.count:SetPoint("BOTTOMRIGHT", 5, 0)

						-- Cooldown
						overlay.cooldown = CreateFrame("Cooldown", nil, overlay)
						overlay.cooldown:SetAllPoints()

						if overlay.cooldown.SetReverse then
							overlay.cooldown:SetReverse(true)
						end

						if overlay.cooldown.SetDrawEdge then
							overlay.cooldown:SetDrawEdge(true)
						end
					...
					if overlay.cooldown.SetScale then
						overlay.cooldown:SetScale(overlay.masqueEnabled and 1 or (overlaySize / 36))
					end
					...
					if bar.showTooltip and not overlay:GetScript("OnEnter") then
						overlay:SetScript("OnEnter", function(s)
							...
							if self.test then
								if s.spellId then
									GameTooltip:SetHyperlink("spell:" .. s.spellId)
								end
							else
...
overlay:RegisterForClicks()
					...
					if overlay.cooldown.SetFrameLevel
					and overlay.cooldown.SetFrameLevel ~= nop then
        ...
        if ShouldShow(bar, frameType) and not (barNameToApply and barName ~= barNameToApply) then
            ...
            while overlayNum <= bar.iconCount do
                ...
                if data then
					...
					local pixelBorderSize = bar.iconBorder and math.max(olay.border.borderSize * 0.75, 0.75) or 1
